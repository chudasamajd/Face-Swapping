...
img = cv2.imread('D:/Python Projects/face_swapping/z.jpg',1)
img2 = cv2.imread('D:/Python Projects/face_swapping/jb2.jpg',1)
...
predictor = dlib.shape_predictor("D:/Python Projects/Face Swapping/shape_predictor_68_face_landmarks.dat/shape_predictor_68_face_landmarks.dat")
...
# Trianuglation for second image from first image
...
for triangle_index in indexes_triangles:
    ...
    wrap_triangle = cv2.warpAffine(cropped_triangle,M,(w,h))
    wrap_triangle = cv2.bitwise_and(wrap_triangle,wrap_triangle,mask=cropped_tr2_mask)

    #Reconstruct destination face
    triangle_area = img2_new_face[y:y+h, x:x+w]
    triangle_area = cv2.add(triangle_area,wrap_triangle)
    
    #triangle_area = cv2.medianBlur(triangle_area,5)

    img2_new_face[y:y+h, x:x+w] = triangle_area
    # RIGHT CODE FOR REMOVE EDGE IS HERE
    img2_new_face = cv2.medianBlur(img2_new_face,3)   
    # RIGHT CODE FOR REMOVE EDGE IS END HERE
    ...
    #wrap_triangle = cv2.bitwise_and(wrap_triangle,wrap_triangle,mask=mask_triangles_designed)
    #my code end
